...
def train(args, device, g, reverse_eids, seed_edges, model, threads):
    # create sampler & dataloader
    
    load_core = list(range(0,4))
    comp_core = list(range(4,4+threads))
    ...
    dataloader = DataLoader(
        ...
        device=device, batch_size=2048, shuffle=True, 
...
if __name__ == '__main__':
    ...
    if n >= 4+4:
        threads = 4
        start = time.time()
        train(args, device, g, reverse_eids, seed_edges, model, threads)
        end = time.time()
        exe_time = end - start
        msg = "4 cores: " + str(exe_time/5) + " sec\n"
        with open("DGL_citation.txt", "a") as text_file:
            text_file.write(msg)
    if n >= 8+4:
        threads = 8
        start = time.time()
    ...
    # use all cores
    threads = psutil.cpu_count(logical = False) - 4 
    start = time.time()
    train(args, device, g, reverse_eids, seed_edges, model, threads)
    end = time.time()
    exe_time = end - start
    msg = str(threads) + " cores: " + str(exe_time/5) + " sec\n"
    with open("DGL_citation.txt", "a") as text_file:
        text_file.write(msg)
