...
def assign_cores(num_cpu_proc):
    ...
    # comp_core = list(range(n//size*rank+num_of_samplers,n//size*rank+num_of_samplers+1))
...
def train(rank, world_size, args, g, data, hidden, meta_data):
    ...
    if args.cpu_process <= 2:
        num_of_samplers = 8//args.cpu_process
    else:
        num_of_samplers = 16//args.cpu_process
...
if __name__ == "__main__":
    ...
    with Manager() as manager:
        ...
        print(meta_data)
        with open('regression_test.csv', 'a', newline='') as file:
